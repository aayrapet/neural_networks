...
class LogisticRegression:
        ...
        def LL_value(self,X,b,y):
                return np.sum(y*np.log(self.sigmoid(X@b))+(1-y)*np.log(1-self.sigmoid(X@b))  )
...
class NN_multi_layer_one_width:
    ...
    def initialise_coefs(self):
        ...
        if self.nb_layers>1:
                b_multi_layers=np.random.normal(loc=0.0, scale=0.01, size=(1, self.nb_layers-1))
                epsilon_multi_layers = np.random.normal(loc=0.0, scale=0.01, size=(X.shape[0], self.nb_layers-1))
                return a1,b0,b_first,epsilon_first,b_multi_layers,epsilon_multi_layers
        
        return a1,b0,b_first,epsilon_first,None,None


    def hidden_layer(self,index,X,b_first,epsilon_first,b_multi_layers,epsilon_multi_layers,derivative=False):
            ...
            while i!=index:
                    ...
                    if i==0:
                            ...
                            lin_comb=h_*b_multi_layers[0][i-1]+epsilon_multi_layers.T[i-1].reshape(-1,1)
                            h_=self.logit_cdf(lin_comb) 
                    i=i+1
                            
            return h_ if not derivative  else self.logit_pdf(lin_comb)
    
    def probability(self,X,b_first,epsilon_first,b_multi_layers,epsilon_multi_layers,a1,b0):
            f_xi=a1*self.hidden_layer(self.nb_layers,X,b_first,epsilon_first,b_multi_layers,epsilon_multi_layers,derivative=False)+b0
            return self.logit_cdf(f_xi)
            
    def gradients(self,y,X,b_first,epsilon_first,b_multi_layers,epsilon_multi_layers,a1,b0):
            ...
            gradient_b0=1
            probability=self.probability(X,b_first,epsilon_first,b_multi_layers,epsilon_multi_layers,a1,b0)
            ...
            for i in range(1,self.nb_layers+1):#from 1 to J layers
                    ...
                    result = np.array([b_multi_layers[0][j-1-i] for j in range(i+1, self.nb_layers+1)])
                    if result.size == 0:
                                result = np.array([1])
                    ...
                    gradient_b=np.sum((y - probability) * a1 *product_phi_s_prime*product_b_s* hidden_previous_layer , axis=0)
                    gradient_epsilon=(y - probability) * a1 *product_phi_s_prime*product_b_s
                    if i==1:
        ...
        #     print(gradient_epsilon_all_storage.shape,"storage")
            ...
            return gradient_b_first_storage,gradient_b_multi_storage,gradient_epsilon_all_storage,gradient_a1_storage,gradient_b0_storage


    def gradient_update_ascent(self,y,X,b_first,epsilon_first,b_multi_layers,epsilon_multi_layers,a1,b0,alpha):
                        ...
                        update_epsilon_first=epsilon_first+alpha*gradient_epsilon_all_storage[:,0].reshape(-1,1)
                        # print((gradient_epsilon_all_storage[:,0]).shape,"first vector di")
                        # print(update_epsilon_first.shape,"updat")

                        if self.nb_layers>1:
                                update_epsilon_multi=epsilon_multi_layers+alpha*gradient_epsilon_all_storage[:,1:]
                                update_b_multi=b_multi_layers+alpha*gradient_b_multi_storage
                                return update_a1, update_b0, update_b_first, update_epsilon_first,update_b_multi,update_epsilon_multi
    ...
    def gradient_ascent(self,X,y,alpha):
                ...
                a1_old,b0_old,b_first_old,epsilon_first_old,b_multi_old,epsilon_multi_old=self.initialise_coefs()


                for_teta_eps_mult_old,for_teta_b_mult_old=self.adjust_vector_dim(epsilon_multi_old,b_multi_old)

                
                teta_old=self.get_theta_no_None(a1_old,b0_old,b_first_old,epsilon_first_old,for_teta_b_mult_old,for_teta_eps_mult_old)
                dif=np.inf
                i=0
                while dif>self.thr:
                        i=i+1
                        ...
                        a1, b0, b_first, epsilon_first,b_multi,epsilon_multi=self.gradient_update_ascent(y,X,b_first_old,epsilon_first_old,b_multi_old,epsilon_multi_old,a1_old,b0_old,alpha)
                        
                        for_teta_eps_mult,for_teta_b_mult=self.adjust_vector_dim(epsilon_multi,b_multi)
                        
                        teta=self.get_theta_no_None(a1,b0,b_first,epsilon_first,for_teta_b_mult,for_teta_eps_mult)
                        dif=np.linalg.norm(teta - teta_old)
                        
                        teta_old=teta
                        a1_old,b0_old,b_first_old,epsilon_first_old,b_multi_old,epsilon_multi_old=a1,b0,b_first,epsilon_first,b_multi,epsilon_multi
                        if  i>self.max_iter:
                                print("reached max iterations, not converged")
                                break
                print(i)                                               
    ...
    def predict_proba(self,X):
                ...
                return self.probability(X,self.b_first,self.epsilon_first,self.b_multi,self.epsilon_multi,self.a1,self.b0)
